...
from policy_engine import check_action_allowed, check_requires_approval
    ...
    for _ in range(MAX_LOOP_ITERATIONS):
        ...
        for call in function_calls:
            ...
            if allowed:
                ...
                # AgentVault-style permission tiering: allowed and
                # blocked are one axis; requires-approval is a second,
                # independent axis. This never stops the action here
                # (there's no human-in-the-loop UI in this batch demo)
                # -- it documents that a human would have to sign off
                # in production, and shows up on the audit trail and
                # dashboard regardless of the eventual outcome.
                needs_approval = check_requires_approval(case, call.name)
                if needs_approval:
                    log_event(
                        txn.transaction_id,
                        "requires_approval",
                ...
                _update_case_state(case, call.name, result, needs_approval)
            else:
...
def _update_case_state(case: RecoveryCase, action_name: str, result: dict, needs_approval: bool = False):
